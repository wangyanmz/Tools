...
- (void)selectedDateLogic {
    switch (self.datePickerMode) {
        ...
        case PGDatePickerModeYearAndMonth:
        {
            ...
            break;
        case PGDatePickerModeYearMonthWeek:
        {
            [self yearMonthWeek_setupSelectedDate];
        }
            break;
...
- (void)setDate:(NSDate *)date animated:(BOOL)animated {
    ...
    switch (self.datePickerMode) {
        ...
        case PGDatePickerModeYearMonthWeek:
        {
            [self yearMonthWeek_setDateWithComponents:components animated:animated];
        }
            break;;
        case PGDatePickerModeDate:
        {
...
- (void)pickerView:(PGPickerView *)pickerView title:(NSString *)title didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    switch (self.datePickerMode) {
        ...
        case PGDatePickerModeYearMonthWeek:
        {
            [self yearMonthWeek_didSelectWithComponent:component];
        }
            break;
        case PGDatePickerModeDate:
        {
...
- (NSString *)pickerView:(PGPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    switch (self.datePickerMode) {
        ...
        case PGDatePickerModeYearMonthWeek:
        {
            ...
            if (component == 2) {
                return [NSString stringWithFormat:@"第%@%@", self.weekList[row], self.weekString];
            }
            return [self.yearList[row] stringByAppendingString:self.yearString];
        }
        case PGDatePickerModeDate:
...
- (NSArray *)weekList {
    if (!_weekList) {
        NSUInteger weekCount = self.dayList.count % 7 == 0 ? self.dayList.count / 7 : self.dayList.count / 7 + 1;
        
        NSMutableArray *weeks = [NSMutableArray array];
        for (NSUInteger i = 1; i < weekCount + 1; i++) {
...
- (NSString *)weekString {
    if (!_weekString) {
        if (!self.isHiddenMiddleText) {
            ...
            _weekString = [NSBundle pg_localizedStringForKey:@"weekString" language:self.language];
        }
    }
    return _weekString;
}
...
- (NSString *)middleWeekString {
    if (!_middleWeekString) {
        _middleWeekString = [NSBundle pg_localizedStringForKey:@"weekString" language:self.language];
    }
